...
//  var difference = allurls.filter(x => !validPatterns.includes(x));
var b = new Set(validPatterns);
var difference = [...allurls].filter(x => !b.has(x));
//

  if (patterns.length) {
    try{
      chrome.webRequest.onBeforeRequest.addListener(blockRequest, {
        urls: difference//validPatterns
      }, ['blocking']);
    } catch (e) {
      console.error(e);
    }
